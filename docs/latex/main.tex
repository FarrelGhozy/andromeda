...
Berdasarkan kebutuhan pengembangan, sistem ANDROMEDA akan diterapkan pada \textbf{6 petak lahan} yang berada dalam \textbf{satu area} (bersebelahan), masing-masing berukuran \textbf{5 m $\times$ 1 m} (total luas 30 m²). Setiap petak akan memiliki:

\begin{itemize}
    \item 1 buah capacitive soil sensor — memonitor kelembaban tanah secara independen
    \item 1 buah solenoid valve 12V NC (kran air otomatis) — mengontrol irigasi per petak
    \item 1 jalur drip irrigation (selang PE 16mm + dripper) — distribusi air merata
\end{itemize}

Panel kontrol utama ditempatkan di \textbf{tepi area lahan}, dengan jarak kabel ke petak terjauh diperkirakan \textbf{10--30 meter}. Semua sensor dan kran dikendalikan oleh \textbf{1 (satu) ESP32 DevKit} yang sama.
...
\centering
\begin{tikzpicture}[
    petak/.style={rectangle, minimum width=1.8cm, minimum height=0.7cm, text centered, draw, font=\footnotesize},
]

% Grid 6 petak (3 baris $\times$ 2 kolom) dalam 1 area
\draw[thick, primaryblue!50, rounded corners] (-0.3, 0.8) rectangle (6.3, -4.7);
\node[above, font=\small] at (3, 0.8) {\textbf{AREA LAHAN (30 m²)}};

\foreach \x in {0,1,2} {
    \foreach \y in {0,1} {
        \node[petak, fill=green!10] at (\y*3, -\x*1.7) {Petak \pgfmathparse{int(\x*2+\y+1)}\pgfmathresult};
    }
}

% Panel kontrol di sisi kiri
\node[rectangle, fill=primaryblue!25, draw=primaryblue, thick, minimum width=1.8cm, minimum height=0.9cm, rounded corners, font=\footnotesize] at (-1.6, -2) {\textbf{Panel}\\ \textbf{Kontrol}};

% Kabel backbone dari panel ke junction box
\node[circle, fill=gray!40, inner sep=2pt, font=\tiny] at (0, -2) {};
\node[above, font=\tiny] at (0, -2) {JB};

\draw[thick, green!50!black] (-0.7, -2) -- (0, -2);
\node[above, font=\tiny] at (-0.35, -2) {Kabel utama};

% Kabel dari junction box ke setiap petak
\foreach \x in {0,1,2} {
    \foreach \y in {0,1} {
        \draw[thick, green!50!black] (0, -2) -- (\y*3-0.9, -\x*1.7);
    }
}

% Keterangan
\node[below, font=\footnotesize] at (3, -5.2) {
\begin{tabular}{l}
\textbullet\ 1 ESP32 DevKit — kontrol semua sensor \& kran \\
\textbullet\ Panel kontrol di tepi area — kabel pendek ke petak \\
\textbullet\ Tandon air di posisi lebih tinggi (gravitasi) \\
\textbullet\ ---------------------------------------------------- \\
$\,$ Ukuran petak: 5m $\times$ 1m \; Lebar area: 10m \; Panjang area: 15m
\end{tabular}
};

% Ukuran
\draw[<->, thick] (6.5, 0.5) -- (0.5, 0.5) node[midway, above, font=\footnotesize] {$\sim$10 m};
\draw[<->, thick] (-0.5, 0.6) -- (-0.5, -0.8) node[midway, left, font=\footnotesize] {1 m};

\end{tikzpicture}
\caption{Tata letak 6 petak lahan — 1 area, 1 ESP32, junction box di tengah}
\label{fig:6-plot-layout}
\end{figure}


\subsection{Arsitektur Sistem: 1 ESP32 — 6 Sensor — 6 Kran}

Karena semua petak berada dalam satu area, arsitektur yang digunakan sangat sederhana:

\begin{itemize}
    \item \textbf{1 ESP32 DevKit} — sebagai satu-satunya mikrokontroler yang membaca semua sensor dan mengontrol semua kran.
    \item \textbf{Junction Box (JB)} di tengah area — tempat percabangan kabel dari panel ke tiap petak. Memudahkan instalasi dan maintenance.
    \item \textbf{Kabel backbone} — satu kabel multi-core (12 pair twisted shielded) dari panel ke junction box (\textasciitilde10--30m). Dari JB, kabel pendek (\textasciitilde1--3m) ke tiap sensor/kran.
\end{itemize}

\subsubsection{Koneksi Pin ESP32}

\begin{table}[H]
\centering
\caption{Koneksi pin ESP32 untuk 6 petak}
\label{tab:esp32-6pin}
\begin{tabularx}{\textwidth}{c c c X}
\toprule
\textbf{GPIO} & \textbf{Fungsi} & \textbf{Terhubung ke} & \textbf{Keterangan} \\
\midrule
32 & ADC1\_CH0 & Sensor 1 (via 4-20mA + 250$\Omega$) & Petak 1 \\
33 & ADC1\_CH1 & Sensor 2 (via 4-20mA + 250$\Omega$) & Petak 2 \\
34 & ADC1\_CH2 & Sensor 3 (via 4-20mA + 250$\Omega$) & Petak 3 \\
35 & ADC1\_CH3 & Sensor 4 (via 4-20mA + 250$\Omega$) & Petak 4 \\
36 & ADC1\_CH4 & Sensor 5 (via 4-20mA + 250$\Omega$) & Petak 5 \\
37 & ADC1\_CH5 & Sensor 6 (via 4-20mA + 250$\Omega$) & Petak 6 \\
\midrule
26 & Digital Out & Relay CH1 $\to$ Solenoid Valve 1 & Petak 1 \\
27 & Digital Out & Relay CH2 $\to$ Solenoid Valve 2 & Petak 2 \\
14 & Digital Out & Relay CH3 $\to$ Solenoid Valve 3 & Petak 3 \\
12 & Digital Out & Relay CH4 $\to$ Solenoid Valve 4 & Petak 4 \\
13 & Digital Out & Relay CH5 $\to$ Solenoid Valve 5 & Petak 5 \\
15 & Digital Out & Relay CH6 $\to$ Solenoid Valve 6 & Petak 6 \\
\midrule
23 & Output & LED Indikator Biru (sistem hidup) & Panel \\
22 & Output & Buzzer / alarm & Panel \\
\bottomrule
\end{tabularx}
\end{table}

\subsubsection{Diagram Blok Sistem 6 Petak}

\begin{figure}[H]
\centering
\resizebox{0.9\textwidth}{!}{%
\begin{tikzpicture}[
    box/.style={rectangle, rounded corners, minimum width=2.2cm, minimum height=0.8cm, text centered, draw, font=\small},
    wire/.style={thick}
]

% ESP32 di tengah
\node[box, fill=primaryblue!25, draw=primaryblue, minimum width=2.5cm, minimum height=1.2cm] (esp) {\textbf{ESP32}\\ \footnotesize 1 unit};

% Sensor (atas)
\foreach \i/\x in {1/0,2/1.5,3/3,4/4.5,5/6,6/7.5} {
    \node[box, fill=orange!15] at (\x, 1.5) {Sensor \i};
    \draw[->, thick, green!50!black] (\x, 1.1) -- (\x, 0.3);
    \node at (\x, 0.7) {\tiny 4-20mA};
}
\node[above, font=\footnotesize] at (3.75, 2) {\textbf{6 $\times$ Capacitive Soil Sensor + XTR116}};

% Relay
\node[box, fill=gray!15] at (3.75, -1.2) {Relay 8-Ch};
\draw[->, thick] (esp.south) -- (3.75, -0.8);

% Valve (bawah)
\foreach \i/\x in {1/0,2/1.5,3/3,4/4.5,5/6,6/7.5} {
    \node[box, fill=cyan!15] at (\x, -2.2) {Kran \i};
    \draw[->, thick, blue!50!black] (\x, -1.6) -- (\x, -1.9);
}
\node[below, font=\footnotesize] at (3.75, -2.7) {\textbf{6 $\times$ Solenoid Valve 12V NC}};

% Power
\node[box, fill=red!15] at (9.5, 0) {Aki 12V};
\node[box, fill=yellow!15] at (9.5, -1.2) {SCC 20A};
\node[box, fill=orange!15] at (9.5, -2.4) {Solar 300Wp};
\draw[->, thick] (9.5, 0.4) -| (esp.east);
\draw[->, thick] (9.5, -0.8) -- (9.5, -0.4);
\draw[->, thick] (9.5, -1.8) -- (9.5, -1.6);

% LTE
\node[box, fill=green!15] at (9.5, 1.2) {LTE Router};
\draw[->, thick] (9.5, 1.6) -| (esp.east);

% Tandon
\node[box, fill=cyan!15] at (3.75, -3.4) {Tandon Air (Gravitasi)};
\draw[->, thick, blue!50!black] (3.75, -2.6) -- (3.75, -3);
\node at (6, -3.4) {$\to$ Drip Irrigation};

\end{tikzpicture}
}
\caption{Diagram blok sistem ANDROMEDA untuk 6 petak — 1 ESP32, 6 sensor, 6 kran}
\label{fig:6-plot-block}
\end{figure}


\subsection{BOM untuk 6 Petak}
...
\midrule
\multicolumn{5}{l}{\textbf{Pusat (Panel Kontrol + Daya)}} \\
...
3 & Huawei B535-932 4G LTE Router & 1 & Rp 400.000 -- 500.000 & \href{https://shopee.co.id/search?keyword=Huawei+B535+router+4G+LTE}{Cari} \\
4 & Aki Kering 100Ah & 1 & Rp 900.000 -- 1.200.000 & \href{https://shopee.co.id/search?keyword=aki+kering+100Ah+NS70}{Cari} \\
5 & Solar Panel 300Wp Poly & 1 & Rp 1.000.000 -- 1.500.000 & \href{https://shopee.co.id/search?keyword=solar+panel+300wp+poly}{Cari} \\
6 & Solar Charge Controller 20A PWM & 1 & Rp 100.000 -- 150.000 & \href{https://shopee.co.id/search?keyword=solar+charge+controller+20A+PWM}{Cari} \\
7 & Power Supply 12V 5A Adaptor & 1 & Rp 40.000 -- 60.000 & \href{https://shopee.co.id/search?keyword=power+supply+12V+5A}{Cari} \\
8 & Step-Down LM2596 (12V$\to$5V untuk ESP32) & 1 & Rp 15.000 -- 25.000 & \href{https://shopee.co.id/search?keyword=LM2596+step+down}{Cari} \\
...
\midrule
\multicolumn{5}{l}{\textbf{Sensor \& 4-20mA (per petak $\times$6)}} \\
10 & Capacitive Soil Sensor v1.2 & 6 & Rp 90.000 -- 150.000 & \href{https://shopee.co.id/search?keyword=capacitive+soil+sensor}{Cari} \\
11 & Modul XTR116 / XTR115 4-20mA TX & 6 & Rp 150.000 -- 210.000 & \href{https://shopee.co.id/search?keyword=XTR116+4-20mA+transmitter}{Cari} \\
12 & Resistor Presisi 250$\Omega$ 0,1\% & 6 & Rp 12.000 -- 30.000 & \href{https://shopee.co.id/search?keyword=resistor+250+ohm+0.1}{Cari} \\
13 & Kapasitor 100nF + 10$\mu$F & 6 pasang & Rp 6.000 -- 12.000 & \href{https://shopee.co.id/search?keyword=kondensator+100nF+10uF}{Cari} \\
\midrule
\multicolumn{5}{l}{\textbf{Aktuasi (per petak $\times$6)}} \\
14 & Solenoid Valve 12V NC 1/2'' & 6 & Rp 300.000 -- 480.000 & \href{https://shopee.co.id/search?keyword=solenoid+valve+12V+NC}{Cari} \\
\midrule
\multicolumn{5}{l}{\textbf{Kabel \& Instalasi}} \\
15 & Kabel Multi-Core 12 Pair Shielded (backbone) & 30m & Rp 300.000 -- 450.000 & \href{https://shopee.co.id/search?keyword=cable+multi+core+shielded+12+pair}{Cari} \\
16 & Kabel NYMHY 2$\times$1,5mm (valve ke panel) & 100m & Rp 100.000 -- 150.000 & \href{https://shopee.co.id/search?keyword=NYMHY+2x1.5+listrik}{Cari} \\
17 & Selang Drip Irrigation PE 16mm & 50m & Rp 100.000 -- 150.000 & \href{https://shopee.co.id/search?keyword=selang+PE+drip+irrigation+16mm}{Cari} \\
18 & Dripper / Emitter 4L/jam & 60 pcs & Rp 60.000 -- 120.000 & \href{https://shopee.co.id/search?keyword=dripper+irigasi+tetes+tanaman}{Cari} \\
19 & Fitting Selang (Tee, Elbow, Connector) & 1 set & Rp 80.000 -- 120.000 & \href{https://shopee.co.id/search?keyword=fitting+selang+irigasi+tetes+16mm}{Cari} \\
\midrule
\multicolumn{5}{l}{\textbf{Perkakas \& Lainnya}} \\
20 & Project Box (junction box outdoor) & 1 & Rp 25.000 -- 40.000 & \href{https://shopee.co.id/search?keyword=project+box+outdoor+elektronik}{Cari} \\
...
\midrule
\multicolumn{4}{l}{\textbf{Total Estimasi Keseluruhan (6 Petak)}} & \textbf{Rp 3.923.000 -- 5.782.000} \\
\bottomrule
\end{tabularx}
\end{table}

\textbf{Catatan:} Biaya ini sudah termasuk panel surya 300Wp dan aki 100Ah yang merupakan investasi jangka panjang. Tanpa solar + aki (jika listrik PLN tersedia), biaya turun menjadi \textbf{Rp 1.883.000 -- 2.932.000}.


\subsection{Kesimpulan untuk 6 Petak}

\begin{itemize}
    \item \textbf{Arsitektur:} 1 ESP32 — 6 sensor (via 4-20mA) — 6 kran (via relay 8-ch). Tidak perlu mikrokontroler tambahan.
    \item \textbf{Total investasi:} Rp \textbf{3,9 -- 5,8 juta} (dengan solar + aki) atau Rp \textbf{1,9 -- 2,9 juta} (tanpa solar + aki).
    \item \textbf{Biaya per petak:} Rp 650.000 -- 960.000 per petak (dengan solar + aki).
    \item \textbf{Biaya bulanan:} Rp 15.000 -- 30.000 (paket data LTE — sama untuk 1 atau 6 petak).
    \item \textbf{Skalabilitas:} ESP32 masih punya sisa 2 channel ADC (GPIO38-39) dan 2 pin digital untuk ekspansi ke 8 petak. Untuk >8 petak, gunakan multiplexer CD4051 atau tambah modul ADC eksternal (ADS1115 via I2C).
    \item \textbf{Daya:} Aki 100Ah = 1.200Wh. ESP32 (0,3W) + router (10W) + 6 valve (maks 3A @ 12V = 36W, hanya saat aktif) = konsumsi harian \textasciitilde{}250--300Wh. Solar panel 300Wp di Indonesia (\textasciitilde{}4-5 sun hours) menghasilkan \textasciitilde{}1.200-1.500 Wh/hari — lebih dari cukup.
\end{itemize}
