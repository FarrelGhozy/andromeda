...
% ============================================================
%  SECTION - SKENARIO IMPLEMENTASI 6 PETAK LAHAN
% ============================================================
\newpage
\section{Skenario Implementasi 6 Petak Lahan}

\subsection{Spesifikasi Lahan}

Berdasarkan kebutuhan pengembangan, sistem ANDROMEDA akan diterapkan pada \textbf{6 petak lahan}, masing-masing berukuran \textbf{5 m $\times$ 1 m} (total luas 30 m²). Setiap petak akan memiliki:

\begin{itemize}
    \item 1 buah capacitive soil sensor — memonitor kelembaban tanah secara independen
    \item 1 buah solenoid valve 12V NC — mengontrol irigasi per petak
    \item 1 jalur drip irrigation (selang PE 16mm + dripper) — distribusi air merata
\end{itemize}

Panel kontrol utama ditempatkan di \textbf{satu titik pusat} dengan jarak kabel ke petak terjauh diperkirakan \textbf{10--50 meter}.


\subsection{Tata Letak Lahan (Layout)}

\begin{figure}[H]
\centering
\begin{tikzpicture}[
    petak/.style={rectangle, minimum width=1.8cm, minimum height=0.5cm, text centered, draw, font=\scriptsize},
    label/.style={font=\small}
]

% Grid 6 petak (3 baris × 2 kolom)
\foreach \x in {0,1,2} {
    \foreach \y in {0,1} {
        \node[petak, fill=green!10] at (\y*3, -\x*1.5) {Petak \pgfmathparse{int(\x*2+\y+1)}\pgfmathresult};
    }
}
...
\foreach \x in {0,1,2} {
    \foreach \y in {0,1} {
        \node[circle, fill=orange!60, inner sep=2pt] at (\y*3-0.6, -\x*1.5+0.2) {};
    }
}
...
\foreach \x in {0,1,2} {
    \foreach \y in {0,1} {
        \node[circle, fill=cyan!60, inner sep=2pt] at (\y*3+0.6, -\x*1.5+0.2) {};
    }
}

% Panel kontrol di tengah
\node[rectangle, fill=primaryblue!20, draw=primaryblue, thick, minimum width=2cm, minimum height=1cm, rounded corners] at (1.5, -2.25) {\textbf{Panel}\\ \textbf{Kontrol}};

% Kabel dari panel ke petak
\foreach \x in {0,1,2} {
    \foreach \y in {0,1} {
        \draw[thick, green!50!black] (1.5, -2.25) -- (\y*3, -\x*1.5);
    }
}

% Keterangan
\node[below right, font=\tiny] at (3.5, 0) {\textbullet \orange{$\bullet$} Sensor};
\node[below right, font=\tiny] at (3.5, -0.3) {\textbullet \cyan{$\bullet$} Solenoid Valve};
\node[below right, font=\tiny] at (3.5, -0.6) {--- Kabel sinyal \& power};

% Ukuran
\draw[<->, thick] (3.2, 0.3) -- (0.2, 0.3) node[midway, above, font=\tiny] {5 m};
\draw[<->, thick] (-0.5, 0.5) -- (-0.5, -0.5) node[midway, left, font=\tiny] {1 m};

\end{tikzpicture}
\caption{Tata letak 6 petak lahan ANDROMEDA — panel kontrol di titik pusat}
\label{fig:6-plot-layout}
\end{figure}


\subsection{Analisis Skalabilitas: 6 Titik Sensor}

Dengan 6 titik sensor yang tersebar, pemilihan arsitektur transmisi sinyal menjadi sangat krusial. Terdapat dua pendekatan yang dipertimbangkan:

\subsubsection{Opsi A: 6 $\times$ 4-20mA Current Loop (Individual)}

Setiap petak memiliki satu modul XTR116 dan satu pasang kabel Belden 3107A terpisah menuju panel kontrol.

\begin{table}[H]
\centering
\caption{Biaya 6 $\times$ 4-20mA Current Loop}
\label{tab:6x4-20ma}
\begin{tabularx}{\textwidth}{l X c}
\toprule
\textbf{Komponen} & \textbf{Perhitungan} & \textbf{Total} \\
\midrule
XTR116 module & 6 $\times$ Rp 35.000 & Rp 210.000 \\
Resistor 250$\Omega$ + kapasitor & 6 $\times$ Rp 3.000 & Rp 18.000 \\
Kabel Belden 3107A (6 $\times$ 50m) & 300m $\times$ Rp 12.000 & \textbf{Rp 3.600.000} ❌ \\
\midrule
\textbf{Total biaya transmisi} & & \textbf{Rp 3.828.000} \\
\bottomrule
\end{tabularx}
\end{table}

\textbf{Kekurangan:} Biaya kabel sangat tinggi karena tiap sensor butuh kabel sendiri-sendiri. Total 300m kabel Belden 3107A.

\subsubsection{Opsi B: RS485 Daisy-Chain Backbone (Rekomendasi 🔥)}

Satu kabel backbone sepanjang lahan menghubungkan semua node sensor secara daisy-chain. Setiap node menggunakan MAX485 + ADS1115.

\begin{table}[H]
\centering
\caption{Biaya RS485 untuk 6 Petak}
\label{tab:rs485-6plots}
\begin{tabularx}{\textwidth}{l X c}
\toprule
\textbf{Komponen} & \textbf{Perhitungan} & \textbf{Total} \\
\midrule
MAX485 module ($\times$2) & 6 $\times$ Rp 20.000 & Rp 120.000 \\
ADS1115 16-bit ADC & 6 $\times$ Rp 17.000 & Rp 102.000 \\
ESP32-C3 SuperMini (node) & 6 $\times$ Rp 30.000 & Rp 180.000 \\
Kabel Belden 3107A backbone & 50m $\times$ Rp 12.000 & \textbf{Rp 600.000} ✅ \\
\cmidrule{2-3}
\textbf{Total biaya transmisi} & & \textbf{Rp 1.002.000} \\
\bottomrule
\end{tabularx}
\end{table}

\textbf{Keunggulan:} Biaya kabel 6$\times$ lebih murah, satu backbone untuk semua node, bisa ditambah sensor lain (suhu, pH) di masa depan.

\subsubsection{Perbandingan Langsung}

\begin{table}[H]
\centering
\caption{Perbandingan 6$\times$4-20mA vs RS485 untuk 6 petak}
\label{tab:6-plot-compare}
\begin{tabularx}{\textwidth}{l X X}
\toprule
\textbf{Parameter} & \textbf{6 $\times$ 4-20mA} & \textbf{RS485 Daisy-Chain} \\
\midrule
Biaya transmisi & \textbf{Rp 3.828.000} ❌ & \textbf{Rp 1.002.000} ✅ \\
Panjang kabel total & 300m & 50m + 6m drop \\
Jumlah kabel ke panel & 6 pasang & 1 backbone \\
Multi-sensor (suhu, pH) & Butuh tambahan loop & Tinggal tambah node \\
Kompleksitas wiring & Tinggi (6 kabel terpisah) & Rendah (1 kabel) \\
Maintenance & Cari kabel putus susah & Deteksi node failure mudah \\
Skalabilitas ke 12 petak & 600m kabel (Rp 7,2jt) ✅ & 50m + 12 node (Rp 2jt) ✅✅ \\
\bottomrule
\end{tabularx}
\end{table}


\subsection{Rekomendasi Arsitektur Final untuk 6 Petak 🔥}

Berdasarkan analisis di atas, rekomendasi untuk implementasi 6 petak adalah:

\subsubsection*{Arsitektur Hybrid: RS485 Backbone + Aktuasi Terpusat}

\begin{enumerate}
    \item \textbf{Sensor:} RS485 daisy-chain — satu kabel Belden 3107A backbone (50m) menghubungkan 6 node sensor. Setiap node terdiri dari:
    \begin{itemize}
        \item Capacitive Soil Sensor $\to$ ADS1115 (ADC 16-bit, I2C)
        \item ESP32-C3 SuperMini (mikrokontroler node, baca ADS1115, kirim via RS485)
        \item MAX485 (RS485 transceiver)
    \end{itemize}
    
    \item \textbf{Aktuasi:} 6 solenoid valve dikontrol dari panel secara terpusat (kabel power valve 2$\times$1,5mm ke tiap petak). ESP32 utama memiliki 6 pin GPIO untuk mengontrol 6 relay module.
    
    \item \textbf{Komunikasi:} Semua data sensor dari 6 petak dikumpulkan oleh ESP32 utama melalui bus RS485, kemudian dikirim ke Supabase via WiFi (Huawei B535).
    
    \item \textbf{Power:} Setiap node sensor mendapat daya 12V dari backbone (tidak perlu baterai). Solenoid valve juga 12V dari panel.
\end{enumerate}


\subsection{Skema Wiring 6 Petak — RS485 Multi-Node}

\begin{figure}[H]
\centering
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
    nodec/.style={rectangle, rounded corners, minimum width=2cm, minimum height=0.8cm, text centered, draw, font=\tiny},
    main/.style={rectangle, rounded corners, minimum width=2.5cm, minimum height=0.8cm, text centered, draw, font=\small}
]

% BACKBONE
\draw[thick, green!50!black] (0,0) -- (14,0);
\node[above, font=\small] at (7,0) {\textbf{RS485 Backbone — Kabel Belden 3107A (50m)}};

% NODE 1-6
\foreach \x in {0,2,4,6,8,10} {
    \pgfmathsetmacro{\petak}{int(\x/2+1)}
    \node[nodec, fill=orange!10] at (\x+1, -1.5) {Petak\\\petak};
    \draw[thick] (\x+1, -0.8) -- (\x+1, -1.2);
    \node[circle, fill=orange!60, inner sep=1.5pt] at (\x+1, -1.2) {};
    \node[circle, fill=cyan!60, inner sep=1.5pt] at (\x+1, -1.8) {};
}

% PANEL UTAMA (kanan)
\node[main, fill=primaryblue!20, draw=primaryblue, thick] at (13, -1.5) {\textbf{Panel}\\ \textbf{Kontrol}};
\draw[thick] (12,0) -- (13,-1);

% Relay ×6
\node[main, fill=gray!10] at (13, -3) {Relay $\times$6};
\draw[->, thick] (13, -2.3) -- (13, -2.7);

% Solenoid valves
\foreach \x in {0,2,4,6,8,10} {
    \draw[->, thick, blue!50!black] (13, -3) -- (\x+1, -1.8);
}

% Power
\node[main, fill=red!10] at (13, -4.5) {Aki 12V \\ 100Ah};
\draw[->, thick] (13, -3.5) -- (13, -4);

% Solar
\node[main, fill=yellow!10] at (11, -5.5) {Solar Panel 300Wp};
\node[main, fill=orange!10] at (13, -5.5) {SCC 20A};
\draw[->, thick] (11, -5) -- (11, -5) -- (12, -5);
\draw[->, thick] (12, -5) -- (13, -5);
\draw[->, thick] (13, -5) -- (13, -4.5);
\node at (12, -5.2) {};

% LTE Router
\node[main, fill=green!10] at (15, -4.5) {LTE\\Router};
\draw[->, thick] (14, -4.5) -- (15, -4.5);

% Keterangan
\node[below, font=\tiny] at (7, -6.2) {
\begin{tabular}{l}
\textbullet\ Backbone: 1 kabel untuk 6 node sensor \\
\textbullet\ Power \& data dalam satu kabel \\
\textbullet\ Solenoid valve via kabel terpisah dari panel
\end{tabular}
};

\end{tikzpicture}
}
\caption{Skema RS485 backbone untuk 6 petak ANDROMEDA}
\label{fig:rs485-6plot}
\end{figure}


\subsection{BOM Update untuk 6 Petak}

\begin{table}[H]
\centering
\caption{Bill of Materials — 6 Petak ANDROMEDA}
\label{tab:bom-6plot}
\begin{tabularx}{\textwidth}{c X c c c}
\toprule
\textbf{No} & \textbf{Komponen} & \textbf{Qty} & \textbf{Estimasi} & \textbf{Link Shopee} \\
\midrule
\multicolumn{5}{l}{\textbf{Pusat (Panel Kontrol)}} \\
1 & ESP32 DevKit Board 30 Pin & 1 & Rp 45.000 -- 60.000 & \href{https://shopee.co.id/search?keyword=ESP32+DevKit+Board+30+pin}{Cari} \\
2 & Relay Module 8-Channel 5V & 1 & Rp 35.000 -- 50.000 & \href{https://shopee.co.id/search?keyword=relay+8+channel+5V}{Cari} \\
3 & MAX485 RS485 Module & 1 & Rp 15.000 -- 25.000 & \href{https://shopee.co.id/search?keyword=MAX485+RS485+module}{Cari} \\
4 & Huawei B535-932 4G LTE Router & 1 & Rp 400.000 -- 500.000 & \href{https://shopee.co.id/search?keyword=Huawei+B535+router+4G+LTE}{Cari} \\
5 & Power Supply 12V 5A Adaptor & 1 & Rp 40.000 -- 60.000 & \href{https://shopee.co.id/search?keyword=power+supply+12V+5A}{Cari} \\
6 & Aki Kering 100Ah (Yuasa NS70) & 1 & Rp 900.000 -- 1.200.000 & \href{https://shopee.co.id/search?keyword=aki+kering+100Ah+NS70}{Cari} \\
7 & Solar Panel 300Wp Poly & 1 & Rp 1.000.000 -- 1.500.000 & \href{https://shopee.co.id/search?keyword=solar+panel+300wp+poly}{Cari} \\
8 & Solar Charge Controller 20A PWM & 1 & Rp 100.000 -- 150.000 & \href{https://shopee.co.id/search?keyword=solar+charge+controller+20A+PWM}{Cari} \\
9 & MCB 6A + Terminal Block + Box Panel & 1 set & Rp 70.000 -- 100.000 & \href{https://shopee.co.id/search?keyword=project+box+panel+listrik}{Cari} \\
\midrule
\multicolumn{5}{l}{\textbf{Per Petak ($\times$6)}} \\
10 & Capacitive Soil Sensor v1.2 & 6 & Rp 90.000 -- 150.000 & \href{https://shopee.co.id/search?keyword=capacitive+soil+sensor}{Cari} \\
11 & ADS1115 16-bit ADC Module & 6 & Rp 90.000 -- 120.000 & \href{https://shopee.co.id/search?keyword=ADS1115+ADC+16+bit}{Cari} \\
12 & ESP32-C3 SuperMini & 6 & Rp 150.000 -- 180.000 & \href{https://shopee.co.id/search?keyword=ESP32+C3+SuperMini}{Cari} \\
13 & MAX485 RS485 Module (node) & 6 & Rp 90.000 -- 150.000 & \href{https://shopee.co.id/search?keyword=MAX485+RS485+module}{Cari} \\
14 & Solenoid Valve 12V NC 1/2'' & 6 & Rp 300.000 -- 480.000 & \href{https://shopee.co.id/search?keyword=solenoid+valve+12V+NC}{Cari} \\
15 & Project Box kecil (node) & 6 & Rp 60.000 -- 90.000 & \href{https://shopee.co.id/search?keyword=project+box+elektronik+kecil}{Cari} \\
\midrule
\multicolumn{5}{l}{\textbf{Kabel dan Irigasi}} \\
16 & Kabel Belden 3107A backbone & 50m & Rp 500.000 -- 600.000 & \href{https://shopee.co.id/search?keyword=Belden+3107A+kabel+shielded}{Cari} \\
17 & Kabel NYMHY 2$\times$1,5mm (valve) & 100m & Rp 100.000 -- 150.000 & \href{https://shopee.co.id/search?keyword=NYMHY+2x1.5+listrik}{Cari} \\
18 & Selang Drip Irrigation PE 16mm & 50m & Rp 100.000 -- 150.000 & \href{https://shopee.co.id/search?keyword=selang+PE+drip+irrigation+16mm}{Cari} \\
19 & Dripper / Emitter 4L/jam & 60 pcs & Rp 60.000 -- 120.000 & \href{https://shopee.co.id/search?keyword=dripper+irigasi+tetes+tanaman}{Cari} \\
20 & Fitting Selang + Tee + Elbow & 1 set & Rp 80.000 -- 120.000 & \href{https://shopee.co.id/search?keyword=fitting+selang+irigasi+tetes+16mm}{Cari} \\
\midrule
\multicolumn{5}{l}{\textbf{Perkakas}} \\
21 & Timah Solder + Flux & 1 set & Rp 15.000 -- 25.000 & \href{https://shopee.co.id/search?keyword=timah+solder+flux+rosin}{Cari} \\
22 & Crimping Tool + Skun Kabel & 1 set & Rp 30.000 -- 50.000 & \href{https://shopee.co.id/search?keyword=crimping+tool+skun+kabel}{Cari} \\
23 & Multimeter Digital & 1 & Rp 50.000 -- 100.000 & \href{https://shopee.co.id/search?keyword=multimeter+digital}{Cari} \\
\midrule
\multicolumn{4}{l}{\textbf{Total Estimasi Keseluruhan (6 Petak)}} & \textbf{Rp 4.220.000 -- 6.110.000} \\
\bottomrule
\end{tabularx}
\end{table}


\subsection{Kesimpulan untuk 6 Petak}

\begin{itemize}
    \item \textbf{Arsitektur terbaik:} RS485 daisy-chain backbone \textbf{(bukan 6 $\times$ 4-20mA)} — menghemat Rp 2,8 juta dibanding 6 current loop individual.
    \item \textbf{Total investasi:} Rp \textbf{4,2 -- 6,1 juta} untuk 6 petak (30 m²) — sudah termasuk panel surya + aki untuk daya mandiri.
    \item \textbf{Biaya per petak:} Rp 700.000 -- 1.018.000 per petak (termasuk panel surya + aki yang dipakai bersama).
    \item \textbf{Biaya bulanan:} Rp 15.000 -- 30.000 (paket data LTE, sama untuk 1 atau 6 petak).
    \item \textbf{Skalabilitas:} Untuk 12 petak, cukup tambah node sensor di backbone yang sama. Biaya tambahan hanya komponen node + selang (tanpa kabel backbone baru).
    \item \textbf{Solar + Aki:} Cukup untuk 6 solenoid valve (total 3A jika semua aktif bersamaan) + ESP32 + router LTE 24/7.
\end{itemize}
